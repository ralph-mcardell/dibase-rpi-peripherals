...
#ifndef DIBASE_RPI_PERIPHERALS_INTERNAL_GPIO_REGISTERS_H
# define DIBASE_RPI_PERIPHERALS_INTERNAL_GPIO_REGISTERS_H
...
namespace dibase { namespace rpi {
  ...
  { namespace internal
    ...
    /// 
    /// Each GPIO pin can be set for use as an input pin, an output pin or one
    /// of up to six alternative functions. Which pins support which
    /// alternative functions is detailed in section 6.2 of the 
    /// <a href="http://www.raspberrypi.org/wp-content/uploads/2012/02/BCM2835-ARM-Peripherals.pdf">
    ...
    ///
    /// There are 54 GPIO pins and many control registers have 1 bit per GPIO
    /// pin and thus come in pairs (e.g. GPSET0, GPSET1 and GPCLR0, GPCLR1)
    /// with the single bit fields for GPIO pins 0..31 in GPxxx0 and those for
    /// pins 32..53 in the lower bits of GPxxx1. The one_bit_field_register
    /// type presents a united interface for such register pairs and provides
    /// commonly used functions that are required to perform various control
    /// functions of the various registers.
      ...
      /// @brief Set a single bit to 1; other bits in the same word set to 0
      ///
      /// Overwrites the one word of the pair containing the single bit field
      /// for the passed bit number with a value having just the requested bit
      /// set to 1. The other word is left untouched. This is useful for
      /// registers such as GPSET0,1 and GPCLR0,1 where a zero bit means no
      /// change and only one word would need to be written to to change a
      /// single pin's state.
      ///
      /// Bits are in the range 0...63, but for GPIO pins only 0...53 should
      /// be used.
      ///
      /// @param[in]  bitnumber Bit number in the register pair looked at as a
      ///                       single value (0..63, not range checked).
    ...
    ///
      ...
      /// @param[in]  pinid   Id number of the GPIO pin to set high.
        void set_pin( pin_id pinid ) volatile
        {
      ...
      /// @param[in]  pinid   Id number of the GPIO pin to set low.
        void clear_pin( pin_id pinid ) volatile
        {
      ...
      /// @return Zero if pin level is low or a non-zero value if it is high.
      ...
      /// @param[in]  pinid Id number of the GPIO pin whose event notification.
        void clear_pin_event( pin_id pinid ) volatile
        {
      ...
      ///                     events will be detected.
        void pin_rising_edge_detect_enable( pin_id pinid ) volatile
        {
          gpren.set_bit( pinid );
        }
      ...
      ///
      /// @param[in]  pinid   Id number of the GPIO pin for which rising edge
      ///                     events won't be detected.
        void pin_rising_edge_detect_disable( pin_id pinid ) volatile
        {
      ...
      ///                     events will be detected.
        void pin_falling_edge_detect_enable( pin_id pinid ) volatile
        {
          gpfen.set_bit( pinid );
        }
      ...
      /// @param[in]  pinid   Id number of the GPIO pin for which falling edge
      ///                     events won't be detected.
        void pin_falling_edge_detect_disable( pin_id pinid ) volatile
        {
      ...
      ///                     events will be detected.
        void pin_high_detect_enable( pin_id pinid ) volatile
        {
          gphen.set_bit( pinid );
        }
      ...
      /// @param[in]  pinid   Id number of the GPIO pin for which high detect
      ///                     events won't be detected.
        void pin_high_detect_disable( pin_id pinid ) volatile
        {
      ...
      ///                     events will be detected.
        void pin_low_detect_enable( pin_id pinid ) volatile
        {
          gplen.set_bit( pinid );
        }
      ...
      /// @param[in]  pinid   Id number of the GPIO pin for which low detect
      ///                     events won't be detected.
        void pin_low_detect_disable( pin_id pinid ) volatile
        {
      ...
      ///
      /// @param[in]  pinid Id number of the GPIO pin for which async rising
      ///                   edge events will be detected.
        void pin_async_rising_edge_detect_enable( pin_id pinid ) volatile
        {
      ...
      ///
      /// @param[in]  pinid Id number of the GPIO pin for which async rising
      ///                   edge events won't be detected.
        void pin_async_rising_edge_detect_disable( pin_id pinid ) volatile
        {
      ...
      ///
      /// @param[in]  pinid Id number of the GPIO pin for which async falling
      ///                   edge events will be detected.
        void pin_async_falling_edge_detect_enable( pin_id pinid ) volatile
        {
      ...
      /// @param[in]  pinid Id number of the GPIO pin for which async falling
      ///                   edge events won't be detected.
        void pin_async_falling_edge_detect_disable( pin_id pinid ) volatile
        {
      ...
      /// GPPUDCLKn descriptions for details. The latter in particular
      /// specifies the underlying sequence that needs to be used.
      ///
      ...
      ///                     clock for.
        void assert_pin_pull_up_down_clock( pin_id pinid ) volatile
        {
...
#endif // DIBASE_RPI_PERIPHERALS_INTERNAL_GPIO_REGISTERS_H
