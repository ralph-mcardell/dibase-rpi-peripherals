...
namespace dibase { namespace rpi {
  ...
  { namespace internal
    ...
    /*53*/, {gpio_special_fn::no_fn         , gpio_special_fn::no_fn         , gpio_special_fn::no_fn         ,
          ...
          result_set make_results(PinSeqT pin_seq, Predicate add_pred)
          {
            result_set_builder results;
            for (auto p : pin_seq)
              {
        ...
        result_set select(pin_id p, select_options opt)
        ...
        result_set select(std::initializer_list<pin_id> ps, select_options opt)
        ...
        result_set select(pin_id p, std::initializer_list<gpio_special_fn> ss)
        { return  make_results
                  ( pin_id_range{p,p}
                  , [ss](gpio_special_fn spl_fn)
                    { return std::find(ss.begin(),ss.end(),spl_fn)!=ss.end();
        ...
        result_set select(std::initializer_list<pin_id> ps, gpio_special_fn s)
        {
