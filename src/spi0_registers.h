...
namespace dibase { namespace rpi {
  ...
  { namespace internal
    ...
    /// fields and flags for SPI0 channel control.
      ...
      ///
      /// @note
      /// A chip id value of 2 is apparently not used.
      ///
      ...
      /// @returns  \c true if first SCLK transition at beginning of data bit,
      ///           \c false if first SCLK transition at middle of data bit.
      ...
      /// @returns  \c true if rest state of clock is high,
      ///           \c false if rest state of clock is low.
      ...
      /// select lines
      ///
      /// Returns value of the common CSPOL field.
      ///
      /// @note
      /// It appears that the value of this field has no effect on chip
      /// select polarity.
      ///
      /// @returns  \c true if chip select lines are active high,
      ///           \c false if chip select lines are active low.
      ...
      /// depending on the \c cs_line parameter value.
      ///
      /// @note
      /// As chip id 2 is apparently not used, the CSPOL2 field value
      /// has no use.
      ///
      /// @param[in] cs_line  Chip select line to return specific polarity
      ///                     field value for [0,2].
      /// @returns  \c true if chip select line is active high,
      ///           \c false if chip select line is active low OR cs_line is
      ///           out of range (not 0, 1 or 2). 
      ...
      /// @returns  \c true if DMA operation enabled and requested are
      ///           generated, \c false if DMA operation disabled and no
      ///           requests are issued.
      ...
      ///
      /// @returns  \c true if an interrupt is generated when RXR==\c true,
      ///           \c false if no such interrupt is generated.
      ...
      ///
      /// @returns  \c true if chip select de-asserted at the end of a DMA
      ///           transfer, \c false if it is not.
      ...
      ///
      /// @returns  \c true if going to read from an SPI device,
      ///           \c false if going to write to an SPI device.
      ...
      ///
      /// @returns  \c true if serial interface behaves as a LoSSI master,
      ///           \c false if serial interface behaves as an SPI master.
      ...
      /// [\ref set_transfer_active(\c false)].
      ///
      /// @returns  \c true if transfer is complete,
      ///           \c false if transfer is in progress or transfers not active.
      ...
      ///
      /// @returns  \c true if receive FIFO contains at least 1 byte,
      ///           \c false if receive FIFO is empty.
      ...
      /// flag value
      ///
      /// @returns  \c true if the transmit FIFO has space for at least 1 byte,
      ...
      /// [\ref set_transfer_active(\c false)].
      ///
      /// @returns  \c true if receive FIFO near to being full,
      ///           \c false if receive FIFO is not full or transfers not active.
      ...
      /// flag value
      ///
      /// @returns  \c true if receive FIFO is full. No serial data will be
      ///           transferred until data is read from the receive FIFO,
      ///           \c false if receive FIFO is _not_ full.
      ...
      /// @returns  \c true if DMA mode enabled in LoSSI mode,
      ///           \c false if not.
      ...
      /// @returns  \c true if writing to the FIFO writes a 32-bit word,
      ///           \c false if writing to the FIFO writes a single byte.
      ...
      /// @returns  \c true if the operation was performed, \c false if it was
      ///           not because chip_id value was not in the range [0,2].
      ...
      /// @param[in]  at_start  Pass \c true for first SCLK transition at
      ///                       beginning of data bit or \c false for first
      ...
      /// @param[in] is_high   Pass \c true for rest state of clock to be high.
      ///                      Pass \c false for rest state of clock to be low.
      ...
      /// select polarity.
      ///
      /// @param[in] is_high  Pass \c true for chip select lines active high.
      ///                     Pass \c false for chip select lines active low.
      ...
      /// depending on the \c cs_line parameter value. Other field bits are
      /// unchanged.
      ///
      /// @note
      /// As chip id 2 is apparently not used, the CSPOL2 field value
      ...
      ///                     field value for [0,2].
      /// @param[in] is_high  Pass \c true for chip select line active high.
      ///                     Pass \c false for chip select line active low.
      /// @returns  \c true if operation performed,
      ///           \c false if operation not performed as cs_line is out
      ...
      /// @param[in] is_active  Pass \c true to place in transfer active state.
      ///                       Pass \c false to place in transfer not active
      ///                       state.
      ...
      /// @param[in]  enabled Pass \c true to enable DMA operation and requests.
      ///                     Pass \c false to disable DMA operations.
      ...
      /// @param[in]  deassert  Pass \c true to have chip select de-asserted
      ///                       at the end of a DMA transfer
      ///                       Pass \c false not to automatically de-assert
      ///                       chip select at the end of a DMA transfer.
      ...
      /// @param[in]  enable  Pass \c true indicate intention to read from a
      ///                     device. Pass \c false to indicate intention to
      ///                     write to a device.
      ...
      /// a LoSSI master or SPI master.
      ///
      /// @param[in]  enable  Pass \c true for the serial interface to behave
      ///                     as a LoSSI master.
      ///                     Pass \c false for the serial interface to behave
      ///                     as an SPI master.
      ...
      /// @param[in]  enable  Pass \c true to enable DMA mode in LoSSI mode.
      ///                     Pass \c false to disable DMA mode in LoSSI mode.
      ...
      /// @param[in]  enable  Pass \c true to enable FIFO writing 32 bit word.
      ///                     Pass \c false and FIFO writes single byte.
      ...
      ///
      /// @note
      /// Note #1: Only for poll / interrupt modes. DMA mode uses a DMA write
      /// channel to implicitly write data in 32-bit words to the SPI0 FIFO
      /// register.
      ///
      /// @note
      /// Note #2: In LoSSI mode use transmit_fifo_write() to write command
      ///          bytes and transmit_fifo_lossi_write() to write parameter
      ///          data. 
      ///
      ...
      ///
      /// @note
      /// Only for poll / interrupt modes. DMA mode uses a DMA write
      /// channel to implicitly write data in 32-bit words to the SPI0 FIFO
      /// register.
      ///
      /// In LoSSI mode data bytes have a 1 bit prefix. LoSSI mode slave device 
      /// commands are prefixed by a 0 so can be written by
      /// transmit_fifo_write() but parameter data have a 1 prefix so
      /// transmit_fifo_lossi_write() has to be used to add it.
      ///
      ...
      ///
      /// @note
      /// Only for poll / interrupt modes. DMA mode uses a DMA read
      ...
      ///
      /// @note
      /// Values will be even, in the range [2,65536]. A value of 
      /// 65536 is represented as a register value of 0.
      ///
      /// @returns SPI0 clock register divisor (CDIV) 16-bit field value, or
      ///          65536 if the CDIV value is 0.
      ...
      ///
      /// @note
      /// Values should be even, in the range [2,65536]. Odd values will
      /// be rounded down.
      ///
      ...
      ///
      /// @note
      /// Relevant in DMA mode only (get_dma_enable()==\c true). Value is
      ...
      ///
      /// @note
      /// Valid only in DMA mode (get_dma_enable()==\c true). Value is
      /// for both transmit and receive.
      ///
      /// @param[in] len  SPI0 DMA data transfer length [0,65535]
      /// @returns  \c true if operation performed,
      ///           \c false if operation not performed as length out of range.
      ...
      ///
      /// @note
      /// Relevant in LoSSI mode only (get_lossi_enable()==\c true). Value
      ...
      ///                       [0..255].
      /// @returns  \c true if operation performed,
      ///           \c false if operation not performed as threshold out of
      ///           range.
      ...
      ///
      /// @returns  Receive FIFO level above which a DREQ signal is 
      ...
      ///
      /// @param[in] threshold  Receive FIFO level above which a DREQ
      ...
      ///
      /// @returns  Receive FIFO level above which a Panic signal is 
      ///           issued to the receive DMA engine.
      ...
      ///
      /// @param[in] threshold  Receive FIFO level above which a Panic
      ///                       signal is issued to the receive DMA engine
      ///                       [0..255].
      /// @returns  \c true if operation performed,
      ///           \c false if operation not performed as threshold out of
