...
namespace dibase { namespace rpi {
  ...
  /// @brief Immutable type for values describing a pin's alternative function.
  ///
  /// Immutable here allows object to be changed only by assignment to another
  /// pin_alt_fn_descriptor object as required by std containers.
  ///
  ...
  ///   - the special function the pin has in the alt function mode
    ...
    ///         gpio_pin_fn::input or gpio_pin_fn::output.
      pin_alt_fn_descriptor(pin_id p, gpio_pin_fn a, gpio_special_fn s)
      : pin_{p}
      , alt_fn_{[](gpio_pin_fn a)->gpio_pin_fn
                  { if (a==gpio_pin_fn::input||a==gpio_pin_fn::output)
                      {
                        throw std::invalid_argument
                              { "pin_alt_fn_descriptor::pin_alt_fn_descriptor:"
  ...
  /// @brief Mutable type used to build up results of pin alt function queries
    class pin_alt_fn_result_set_builder
    {
      typedef std::vector<pin_alt_fn_descriptor>   descriptor_container_type;
    ...
    /// @brief Add copy of pin_alt_fn_descriptor object to builder 
    /// @param d  Result object to add  copy of to builder object
      void add(pin_alt_fn_descriptor const & d) { items.push_back(d); }

    /// @brief Add pin_alt_fn_descriptor object constructed in place to builder
    /// @param p  Pin id of GPIO pin alt function of pin_alt_fn_descriptor
    /// @param a  Alt function of GPIO pin of pin_alt_fn_descriptor
    /// @param s  Special function value of pin_alt_fn_descriptor.
    /// @throws std::invalid_argument if gpio_pin_fn argument is 
    ///         gpio_pin_fn::input or gpio_pin_fn::output.
    ...
    /// @returns  std::vector<pin_alt_fn_descriptor> containing builder object's
    ///           contained items.
  ...
  /// @brief Immutable type used to present results of pin alt function queries
    class pin_alt_fn_result_set
    {
      typedef std::vector<pin_alt_fn_descriptor>   descriptor_container_type;
      descriptor_container_type     items; ///< result items collection
    ...
    /// @brief Create from pin_alt_fn_result_set_builder.
    /// The pin_alt_fn_result_set_builder object is drained thus post condition
    /// is that b.empty()==true
    /// @param  b   Builder object containing pin_alt_fn_descriptor results
      explicit pin_alt_fn_result_set( pin_alt_fn_result_set_builder & b )
      : items{b.drain()}
      {}

    /// @brief Number of result items held
    /// @returns Number of pin_alt_fn_descriptor items in result set
      size_type size() const noexcept { return items.size(); }
    ...
    /// @returns Const_iterator to first result item in result set or end()
    ///          if empty()==true.
      const_iterator begin() const noexcept { return items.begin(); }

    /// @brief Const iterator to on epast the last result
    /// @returns Const_iterator to one past the last result item in result set
